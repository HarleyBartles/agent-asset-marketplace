...
Repo-specific anti-slop profiles are behavioral contracts. Repo-wide profiles live under `.agents/contracts/unslop/`; a justified subsystem overlay lives at `<scope>/.agents/contracts/unslop/`. The portable `unslop-profiles` skill owns generic reusable profiles, not consumer-specific contract content.
