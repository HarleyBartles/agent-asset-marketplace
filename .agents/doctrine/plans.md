...
- Keep an active plan current enough that another worker can distinguish finished work from the next action. Checkbox bookkeeping is an in-flight aid, not a publication artifact or permanent reporting duty.
- Order implementation tasks so that all source and adapter/overlay edits are complete before any `tools/run * --apply` regeneration step. Run `tools/run installed-skills --check` after overlay edits and before regeneration to catch source/overlay drift early.
...
This file must stay aligned with the repo's plan documentation practices. When plan formats change or new guidance is needed for plan structure, review and update this file to reflect current expectations. After completion, remove all finished planning artifacts under the completed-artifact custody doctrine.
