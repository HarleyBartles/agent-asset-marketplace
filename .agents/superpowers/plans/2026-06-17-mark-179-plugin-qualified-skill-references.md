...
**Architecture:** Update only the source seams proven by the MARK-178 inventory. The source skill docs should name cross-plugin targets with their plugin-qualified identity, while GPT overlay/export prompt data stays untouched. Add a targeted validator test that enforces the known cross-plugin case without forbidding same-plugin bare references.

**Tech Stack:** Markdown skill sources, Python marketplace validator, unittest.

---

### Task 1: Normalize the cross-plugin receipt reference

**Files:**
- Modify: `codex-marketplace/plugins/house-skills/skills/codex-receipts-superpowers/SKILL.md`
...
```md
Use `repo-worker-base:codex-repo-receipts` for the exact artifact contract, default paths, and narrow omission rule.
```
...
git commit -m "fix: canonicalize cross-plugin receipt reference"
```

### Task 2: Add a targeted validation guard
...
git commit -m "test: guard canonical cross-plugin skill references"
```

### Task 3: Validate the repo surface
