...
- `mark-ready-for-ci` keeps its three-phase shape minus challenge/execution machinery: persist strict intent (idempotency key still derived from canonical intent), witness the `gh pr ready` call, then verify via a `remote-observation` re-fetch that the PR actually left draft on the exact head SHA; a crash leaves a retryable intent reconciled by observation, and an already-ready remote produces a verified no-op. Finalization does not re-run the lawful-action gate: its safety rests on the kernel invariant that any blocking state change (new findings, invalidated proofs, opened blockers, snapshot drift) advances `snapshot_epoch`/`snapshot_fingerprint` or invalidates the `ready_transition` id through a repair cut, so a pre-change intent always fails the currentness check.
