...
- If a single finding goes through more than five fix rounds, route to `blocked` for human adjudication.
...
- `codex-marketplace/plugins/superpowers-plus/skills/iterative-review/references/review-metrics-schema.json` — add `regression_class` and `lens` to the `regressions` and `rounds_per_finding` entries.
...
- `codex-marketplace/plugins/superpowers-plus/skills/selecting-a-subagent/assets/implementer.md` and/or `implementer-strong.md` — if needed, reference the `iterative-review` fix-brief contract.
