...
Phase 3: Retire repo-index/ (completed)
...
The `repo-index/` directory and its `README.md`/`INDEX.md` surfaces were retired. The guidance was moved to `.agents/docs/repo-index.md`, the root `INDEX.json` registry is in place, and the per-zone sidecars are generated by `tools/generate_repo_index.py`.
