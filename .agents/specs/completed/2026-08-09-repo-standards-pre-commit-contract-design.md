...
- `codex-marketplace/plugins/repo-worker-pack/skills/repo-standards/references/repository-shape-manifest.json` (manifest surface definitions; the `source`/`kind` fields already match the new contract, so no edit is needed)
...
Update `repository-shape-standard.md` so the pre-commit hook surface is described as a contract, not a pinned file copy. Remove or reword language that claims the hook is generated byte-for-byte from the template. `repository-shape-manifest.json` requires no behavioral change because its `source` and `kind` fields already describe the contract correctly.
