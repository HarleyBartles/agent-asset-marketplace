...
Field names carry grammar. Follow `.agents/contracts/skill-frontmatter.md` in repositories that provide it: `scope` names ownership, trigger-list values answer their keys without repeating them, and relationship fields contain skill identifiers. For Codex wrappers, `short_description` is capability copy and `default_prompt` directly instructs the already-selected skill.
...
Invoke bundled scripts through their interpreter in the prose (`bash scripts/tool.sh`, `node scripts/tool.js`), never by bare path: some harness plugin packagers strip executable bits, and a bare script path fails there.
