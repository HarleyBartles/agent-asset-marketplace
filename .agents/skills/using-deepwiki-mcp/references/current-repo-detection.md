...
1. Run `git remote -v`.
1. Pick `origin` unless the question is about upstream policy, then use `upstream`.
1. Convert the URL to `owner/repo`:
   - HTTPS: `https://github.com/owner/repo.git` → `owner/repo`
   - SSH: `git@github.com:owner/repo.git` → `owner/repo`
1. If the remote cannot be parsed, or the checkout is not a git repo, ask the user for `owner/repo` explicitly.
