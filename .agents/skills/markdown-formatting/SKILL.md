...
The native formatter configuration must keep prose unwrapped, use LF, validate output, and enable GFM plus YAML frontmatter. The active interpreter must provide the exact versions in `requirements.txt`; hooks never install them dynamically.
