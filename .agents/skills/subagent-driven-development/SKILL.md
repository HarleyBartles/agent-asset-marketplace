...
- **Task brief:** before dispatching an implementer, run this skill's
  `subagent-workspace/scripts/task-brief PLAN_FILE N` — it extracts the task's full text to a
...
**DONE:** Generate the review package (`subagent-workspace/scripts/review-package PLAN_FILE BASE HEAD`, from this skill's directory — it prints the unique file path it wrote; BASE is the commit you recorded before dispatching the implementer — never `HEAD~1`, which silently drops all but the last commit of a multi-commit task), then dispatch the task reviewer with the printed path.
...
- Hand the reviewer its diff as a file: run this skill's
  `subagent-workspace/scripts/review-package PLAN_FILE BASE HEAD` (or `subagent-workspace/scripts/review-package.ps1`
...
**The re-review is scoped.** Run `subagent-workspace/scripts/review-package PLAN_FILE FIX_BASE HEAD`
...
(`subagent-workspace/scripts/review-package PLAN_FILE FIX_BASE HEAD` over the fix range,
