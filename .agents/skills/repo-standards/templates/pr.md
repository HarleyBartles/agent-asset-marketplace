...
- For a normal commit, stage the intended tree and let the tracked pre-commit hook run the consumer's canonical apply capability and then its canonical check capability on the exact staged snapshot. Do not run the full CI check before a normal commit.
