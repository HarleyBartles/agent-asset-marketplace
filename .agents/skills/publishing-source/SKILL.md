...
Owns the source-publication decision tree for this repo: pick the smallest sufficient publication surface for the change, then hand off to the skill that performs the mechanics.
...
- Treating a local commit hash as publication proof. Local state is not repo completion.
- Publishing before `ci --check` is green on the staged tree.
- Editing generated plugin surfaces by hand instead of regenerating from the registry.
