...
2. **Locate.** Resolve each session to verified absolute filesystem paths using
   ...
   a repository-segregated off-repo diagnostic workspace through the host or
   repository scratch policy, tell your partner the path, and fill
   `templates/case.md` there, following its
...
3. **Triage.** Read the region around the reported problem yourself. Then
   use `selecting-a-subagent` and `dispatching-parallel-agents` when delegation
   is authorized, dispatching one analyst subagent per dimension in parallel, each given the
   ...
   Split a dimension by turn range when the transcript is long. Dispatch
   routing does not itself authorize delegation. When runtime or task policy
   forbids fan-out, use a clearly disclosed sequential/self-analysis fallback
   that applies the same seven prompts. Do not claim the analysts ran when they
   did not. Discard any returned finding without `path:line`.
...
5. **GitHub issues** — when report §7 says possible or likely, or your
   partner asks. Route GitHub inspection through `using-github-mcp` and any
   mutation through `connector-safety`. Search open and closed issues for the symptoms per
...
6. **Export** — route sensitive filesystem or archive side effects through
   `connector-safety`. Only proceed when your partner asks for a bundle; never build one
...
- **Context safety.** One transcript line can be a megabyte. Invoke
  `context-safety` and follow `references/context-safety.md` on every session
  file, every time.
