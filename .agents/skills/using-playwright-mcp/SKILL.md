...
1. **Confirm absence before exit.** Before opening `references/other-playwright-tools.md`, check `references/surface-map.md` or run `mcp_list_tools` for `mcp-playwright` to confirm the needed tool is not there.
1. **Environment check for fallbacks.** Before using a non-MCP Playwright tool, run `inspecting-the-environment` to confirm it is installed and available.
