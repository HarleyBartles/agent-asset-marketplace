...
1. The task needs Playwright API patterns that only the Node/Python library supports.
1. The human partner explicitly asked for a standalone Playwright script.

## Procedure

1. Confirm absence with `mcp_list_tools` for `mcp-playwright`.
1. Run `inspecting-the-environment` to confirm Playwright is installed.
1. Choose the right non-MCP surface:
...
1. Keep the script near the task; clean it up after use.
