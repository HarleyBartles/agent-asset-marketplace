...
from _profile_common import load_json  # noqa: E402
...
def _preserve(pattern_id: str, text: str, context: str) -> bool:
    ...
    if pattern_id in {"low-information-affirmation-cluster", "repeated-contrast-profundity"}:
        return "real from simulated" in combined or (
            "customer records" in combined and "generated fixtures" in combined
        )
    ...
    if pattern_id in {"polished-low-information-density", "task-voice-convergence"}:
        return "authorised task voice" in combined or (
            "evidence is incomplete" in combined and "third is missing" in combined
        )
    return False
