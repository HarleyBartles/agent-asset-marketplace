...
4. Materialize inputs as files the subagents can read in the off-repo `iterative-review-<pr_number>` directory:
   ...
   - `<scan_findings>` (optional but strongly recommended): run `py -3 tools/run.py review-preflight --check --base-ref <base>` and `py -3 tools/run.py ci --check` before Round 1, capture their output, and write it to a file. The `review-preflight` target runs the same pattern-based checks Devin auto review tends to find: real Discord guild/server IDs, tokens, email addresses, `license:` nested under `metadata`, stale cross-skill script paths, malformed markdown table rows, `py -m` without `py -3`, `new_plugin.py` exit-code and default-enablement bugs. Include these in the inputs so `reviewer-strong` can cross-check rather than rediscover them.
