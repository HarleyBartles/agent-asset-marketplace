...
3. Materialize inputs in the off-repo `iterative-review-<pr_number>` directory:
   ...
   - `<scan_findings>`: the consumer repo's canonical preflight output as a file. Use the command named in the consumer's `AGENTS.md` or `.devin/rules`, or the installed `capture_preflight.py` helper: `py -3 .agents/skills/iterative-review/scripts/capture_preflight.py --worktree <worktree> --output <scan_findings_path> --command "py -3 tools/run.py review-preflight --check" --command "py -3 tools/run.py ci --check"` (adjust the command strings to the consumer's canonical preflight/CI commands).
