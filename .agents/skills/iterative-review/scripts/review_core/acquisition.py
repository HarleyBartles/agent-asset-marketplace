...
def _required_check_digest(run_gh, repo_id: str, base_ref: str) -> str:
    rc, out, _err = run_gh(
        ["api", f"repos/{repo_id}/branches/{base_ref}/protection"])
    if rc != 0:
        return model.sha256_json({"contexts": [], "enforced": False})
    try:
        doc = json.loads(out)
        contexts = sorted(
            (doc.get("required_status_checks") or {}).get("contexts") or [])
    except Exception:
        raise AcquisitionError(
            "tool-blocked", "branch protection response malformed")
...
def _pr_metadata_projection(pr: dict) -> dict:
    ...
    numbers = sorted(
        i["number"] for i in issues if isinstance(i, dict) and "number" in i)
    labels = sorted(
        lbl.get("name", "")
        for lbl in (pr.get("labels") or []) if isinstance(lbl, dict))
    assignees = sorted(
        a.get("login", "") for a in (pr.get("assignees") or []) if isinstance(a, dict))
...
def _load_seed_bytes(seed, *, run_git, run_gh, base_sha: str, repo_id: str,
                     pr_meta: dict) -> bytes:
    loc = seed.locator
    if loc.startswith("repo:"):
        rc, out, err = run_git(["show", f"{base_sha}:{loc[5:]}"])
        if rc != 0:
            raise AcquisitionError("authority-missing",
                                   f"{loc}: {err.strip() or 'unreadable'}")
    ...
    if loc.startswith("gh:doc/"):
        path = loc[7:]
        rc, out, err = run_gh(
            ["api", f"repos/{repo_id}/contents/{path}?ref={base_sha}"])
        if rc != 0:
            raise AcquisitionError("authority-missing",
                                   f"{loc}: {err.strip() or 'unreadable'}")
        doc = json.loads(out)
...
def enumerate_acquisition(
    *,
    ...
    if rc != 0:
        raise AcquisitionError("tool-blocked",
                               f"gh auth status: {err.strip() or 'unauthenticated'}")

    # 2. PR identity
    rc, out, err = run_gh(["repo", "view", "--json", "nameWithOwner"])
    if rc != 0:
        raise AcquisitionError("tool-blocked",
                               f"gh repo view: {err.strip()}")
    try:
        repo_id = json.loads(out)["nameWithOwner"]
    except Exception as exc:
        raise AcquisitionError("tool-blocked",
                               f"gh repo view malformed: {exc}") from exc
    rc, out, err = run_gh(
        ["pr", "view", str(pr_number), "--json", _PR_JSON_FIELDS,
         "-R", repo_id])
    if rc != 0:
        raise AcquisitionError("tool-blocked",
                               f"gh pr view: {err.strip()}")
    try:
        pr_meta = json.loads(out)
    except Exception as exc:
        raise AcquisitionError("tool-blocked",
                               f"gh pr view malformed: {exc}") from exc
    ...
    if rc != 0:
        raise AcquisitionError(
            "snapshot-drift",
            f"head {head_sha} is not remote-reachable: {err.strip()}")

    # 3. base resolution + shallow check
    rc, out, err = run_git(["rev-parse", "--is-shallow-repository"])
    if rc != 0 or out.strip() != "false":
        raise AcquisitionError("snapshot-drift",
                               "repository is shallow or unreadable")
    ...
    if rc != 0 or len(bases) != 1:
        raise AcquisitionError(
            "snapshot-drift", f"expected exactly one merge base, got {bases}")
    merge_base = bases[0]
    ...
    if rc != 0 or out.strip():
        raise AcquisitionError(
            "snapshot-drift", f"worktree not clean: {out.strip()[:120]}")
    rc, out, err = run_git(["rev-parse", "HEAD"])
    if rc != 0 or out.strip() != head_sha:
        raise AcquisitionError(
            "snapshot-drift",
            f"checked-out HEAD {out.strip() or '<error>'} != pr head {head_sha}")
    ...
    if rc != 0 or obj_format not in model.GIT_OBJECT_FORMATS:
        raise AcquisitionError("snapshot-drift",
                               f"unsupported object format {obj_format!r}")
    rc, out, err = run_git(["rev-parse", f"{head_sha}^{{tree}}"])
    if rc != 0:
        raise AcquisitionError("snapshot-drift",
                               f"tree resolution failed: {err.strip()}")
    tree_sha = out.strip()
    rc, out, err = run_git(["diff", merge_base, head_sha])
    if rc != 0:
        raise AcquisitionError("snapshot-drift",
                               f"diff failed: {err.strip()}")
    ...
    try:
        ...
        seeds, disc_failures = discovery_policy.enumerate_authorities(
            policy=disc, run_git=run_git, base_sha=base_sha,
            pr_metadata={
                "number": pr_number,
    ...
    if disc_failures:
        raise AcquisitionError(
            "authority-missing", json.dumps(disc_failures, sort_keys=True))
    ...
    for i, seed in enumerate(seeds):
        aid = "authority:" + model.sha256_json(
            {"kind": seed.kind, "locator": seed.locator})
        try:
            raw = _load_seed_bytes(
                seed, run_git=run_git, run_gh=run_gh,
                base_sha=base_sha, repo_id=repo_id, pr_meta=pr_meta)
            ...
            entry = {
                "authority_id": aid, "kind": seed.kind, "locator": seed.locator,
            ...
            authorities_records.append({
                **entry, "failure_evidence_id": f"@failure-{i}",
            })
            continue
        sha = write_evidence(f"authority-{i}", "authority", raw)
        entry = {
            "authority_id": aid, "kind": seed.kind, "locator": seed.locator,
            "availability": "loaded", "sha256": sha,
            "failure_class": None, "failure_sha256": None,
        }
        manifest_entries.append(entry)
        authorities_records.append({**entry, "evidence_id": f"@authority-{i}"})

    # 8. feedback history
    try:
        items = feedback_policy.enumerate_feedback(
            run_gh=run_gh, pr_url=pr_meta["url"])
    ...
    for j, item in enumerate(items):
        ...
        aid = "authority:" + model.sha256_json(
            {"kind": "review-feedback", "locator": item.canonical_id})
        entry = {
            "authority_id": aid, "kind": "review-feedback",
            "locator": item.canonical_id, "availability": "loaded",
            "sha256": item.bytes_sha256,
            "failure_class": None, "failure_sha256": None,
        }
        manifest_entries.append(entry)
        authorities_records.append({**entry, "evidence_id": f"@{alias}"})
    ...
    required_checks = _required_check_digest(
        run_gh, repo_id, pr_meta.get("baseRefName") or "main")
    ...
    enumeration_id = model.sha256_json(
        model.authority_discovery_subject(snap_with_fp, manifest_payload))
    manifest_bytes = model.canonical_json(manifest_payload)
    write_evidence("manifest-payload", "authority-manifest-payload",
                   manifest_bytes)
    ...
    (out_dir / "enumeration.json").write_text(
        json.dumps({"enumeration_id": enumeration_id}), encoding="utf-8")
...
class LiveAuthorityDiscovery:
    ...
    def _load_dir(self):
        ...
        ev_manifest = json.loads(
            (self._dir / "evidence" / "manifest.json").read_bytes())
        sources = []
        for alias, rec in sorted(ev_manifest.items()):
            path = self._dir / "evidence" / rec["file"]
            if not path.is_file() or \
                    model.sha256_hex(path.read_bytes()) != rec["sha256"]:
                raise AcquisitionError(
                    "tampered-source", f"evidence {alias} digest mismatch")
            sources.append(engine.EvidenceSource(
                alias=alias, kind=rec["kind"], path=path))
        return data, sources
    ...
    def _find_segment(self, subject_sha: str):
        ...
        for path in sorted(root.glob("*.jsonl"),
            ...
            for idx, line in enumerate(lines):
                ...
                haystack = " ".join(
                    str(v) for v in _walk_strings(tool_input))
        ...
        if not candidates:
            raise AcquisitionError(
                "missing-source",
                "no witnessed enumerate transcript segment for this acquisition")
    ...
    def acquire(self, *, action: str, current_snapshot: dict | None):
        ...
        if action == "refresh-review-input":
            if current_snapshot is None:
                raise AcquisitionError(
                    "missing-snapshot", "refresh requires a current snapshot")
            expected_epoch = current_snapshot["epoch"] + 1
            if snapshot.get("epoch") != expected_epoch:
                raise AcquisitionError(
                    "epoch-mismatch",
                    f"acquisition dir epoch {snapshot.get('epoch')} "
                    f"!= required {expected_epoch}")
            drift_reasons = [
                _DRIFT_NAME_MAP.get(f, f)
                for f in model.SNAPSHOT_SUBJECT_FIELDS
                if f != "epoch"
                and snapshot.get(f) != current_snapshot.get(f)
            ]
        elif action == "freeze-review-input":
            if snapshot.get("epoch") != 1:
                raise AcquisitionError(
                    "epoch-mismatch", "freeze requires an epoch-1 acquisition")
        else:
        ...
        witness["witness_id"] = model.derived_id(
            "witness", snapshot["epoch"],
            model.witness_record_subject(witness))
        wrapper = {
        ...
        return engine.TrustedActionPayload(
            model.canonical_json(envelope), tuple(sources))
