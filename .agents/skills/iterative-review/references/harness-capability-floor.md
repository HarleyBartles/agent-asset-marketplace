...
`reviewctl doctor` re-checks every row at review intake; any missing capability
makes the skill inert for that session (`unsupported-runtime` /
`capability-floor-failed`) rather than silently degrading to assertion-based
evidence.
...
```text
<scratch-root>/<repo>/codex-iterative-review-evidence-kernel/capability-floor/
...
| Hook records carry `session_id`, `prompt_id`, `tool_name`, `tool_input`, `tool_use_id`, `hook_event_name` | PASS | recorded: field set verified verbatim on all records; `session_id` = `shell-quicksand` throughout |
...
| `session_id` survives IDE restart; `prompt_id` rotates per prompt | PASS | recorded + live: `shell-quicksand` across restart; 16 distinct `prompt_id` values |
...
| `gh` authenticated; check-run/workflow-run remote observation on a pushed SHA | PASS | live: `gh auth status` active; check-run `103534879658` (app `github-actions`, `head_sha` `691f8b3fe...`, conclusion `skipped`); workflow-runs carry `id`, `run_attempt`, `run_number`, `head_sha`, `conclusion` |
| `gh pr ready` transitions draft to ready; idempotent; reversible via GraphQL `convertPullRequestToDraft` | PASS | recorded: disposable PR #313 exercised the full transition, repeat call exited 0 ("already ready"), re-fetch via `isDraft` reconciled, re-draft mutation verified |
...
| Review scratch creatable under canonical off-repo root | PASS | live: `Z:\_agent-scratch\agent-asset-marketplace\codex-iterative-review-evidence-kernel\capability-floor\` created, artifacts written |
...
3. **Hook config loads at session start.** Installing or removing hooks
   mid-session has no effect until restart. `doctor` can only verify live
   emission when the hooks pack was installed before the session began;
   otherwise it reports `capability-floor-failed: hooks-not-loaded` with a
   remediation pointer (install pack, restart session).
4. **Model identity is not self-declared.** `subagent_explore`'s prompt
   ...
   frontmatter at the user-global root, verified by the doctor, not from
   agent self-report.
