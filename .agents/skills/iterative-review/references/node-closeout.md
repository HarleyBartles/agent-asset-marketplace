...
1. Confirm the plan's work is complete and promote enduring decisions into ADRs or current doctrine.
1. Remove the completed plan, spec, roadmap, checkpoint, and related planning artifacts from Git. An optional copy may go to the consumer's central completed-artifact scratch store; it is disposable and not evidence.
1. Run the consumer's canonical mesh and marketplace regeneration helpers.
1. Run the consumer's canonical CI check; do not proceed if it fails.
1. Commit the completed-artifact removal normally.
