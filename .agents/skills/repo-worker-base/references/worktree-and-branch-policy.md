...
Read before repository work, worktree creation, branching, scratch use, a PR, or publication decision.

## Portable location algorithm

Use this exact Git-derived algorithm. Future tooling must consume this algorithm rather than deriving repository locations from the Python source file, process directory, or arbitrary parent searches.
...
The top level of `_agent-scratch` is namespaced by `repository-name` so that multiple repositories cannot mix their scratch. Each repository folder then contains branch-scoped subfolders, and plan-scoped scratch lives inside the branch folder.

Run the superproject command first from the actual start path. Any non-empty result is a submodule: reject it unconditionally. Do not allow a shared-checkout override or fallback inference to bypass that rejection.
...
- the external worktree and scratch roots selected under local repository policy.
...
The top level must equal the intended linked worktree, the branch must equal the task branch, the worktree list must register that path, and edited files must resolve below it. Preserve and report pre-existing dirty state.
...
- the current top level, registered worktree, branch, or file paths point at a different checkout;
...
Correct location mismatches before continuing. Do not create a nested worktree or silently switch to the main checkout.

## Worker boundary

Use a dedicated worktree and task branch unless the user explicitly authorizes another route. Before mutation, record the checkout, branch, base commit, and initial status; preserve pre-existing dirty state. Keep disposable artifacts under external_scratch_root, never inside the repository. Scratch is external, per-repository, per-branch, disposable, and never durable custody.

Fetch the required base, make a focused commit, push the branch, and open a PR unless direct-main work is explicitly authorized. Local edits, test logs, and commit hashes are not publication proof; GitHub-visible PR or authorized direct-main evidence is.

## Branch and PR gate

Use a task branch. The normal publication sequence is edit, validate, commit, push, open or update a PR into the approved base, then verify the published head and remote checks. Direct-main mutation requires explicit current authorization. Do not substitute local credentials workarounds for the repository's declared GitHub publication route.
