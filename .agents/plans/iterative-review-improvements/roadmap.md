...
| 3 | Lens dispatch, write-safety, and final polish | in_progress | [Plan 3](2026-08-09-plan-3-lens-dispatch-and-polish.md) | — | — | 9/10 | `select_lenses.py`, self-review template, tests, docs, plus a no-hand-write contract for all scratch files (`review-state.json`, `*.jsonl`, `review-log-*.md`) and a script for orchestrator markdown logs |
| 4 | TDD and fast-fix churn reduction | proposed | — | — | — | — | Enforce `test-driven-development` for implementers and orchestrators, add TDD to implementer profiles and subagent-driven-development prompts, and tighten the `reviewer-fixes`/`fast-fix` node recipes so fixes are proven by failing tests and re-run only the originating lens, not a full final branch review. |
