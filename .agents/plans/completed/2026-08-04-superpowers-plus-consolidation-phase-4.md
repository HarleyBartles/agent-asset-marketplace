...
**Files:**
- Read:
  - `.agents/doctrine/mesh-policy.md` — current mesh law for `.devin/rules/`, `.agents/`, `AGENTS.md`
...
- [x] **Step 1: Add the `.agents/doctrine/` section to mesh-policy.md.**
  ...
  Append the following section after the existing last numbered section (`## 6. Mesh self-healing`) in `.agents/doctrine/mesh-policy.md`. Do not renumber the earlier sections.
...
- [x] **Step 2: Create `.agents/doctrine/AGENTS.md`.**
  ...
  - For the mesh law that governs this surface, read `.agents/doctrine/mesh-policy.md`.
...
- [x] **Step 4: Commit the mesh policy, scoped AGENTS.md, and root AGENTS.md.**
  ...
  ```bash
  git add .agents/doctrine/mesh-policy.md .agents/doctrine/AGENTS.md AGENTS.md
  git diff --stat
