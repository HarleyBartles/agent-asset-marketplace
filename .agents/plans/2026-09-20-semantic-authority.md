...
- [ ] Add focused assertions for the authority-provenance rule, bounded bootstrap, debugging semantic-delta stop, staged-snapshot semantic firewall, and cheap commit loop.
- [ ] Add one pressure scenario reproducing the temptation to infer “staged snapshot means do not roll marketplace source.”
- [ ] Run the focused tests and record the expected RED failures before changing canonical instruction sources.
...
- [ ] Add the model-neutral distinction: evidence changes belief; only authority changes workflow meaning.
- [ ] Require provenance for any new mode, branch, exception, flag, ceremony, or validation semantics.
- [ ] Stop bootstrap expansion when no unresolved question can change the next lawful action.
- [ ] Make semantic expansion a debugging red flag and clarify that reproduced failure authorizes repair, not new policy.
- [ ] State that staged-snapshot markers identify the candidate tree and do not alter consumer command or marketplace-refresh semantics.
- [ ] Express the existing cheap commit loop without creating a second validation path.
...
- [ ] Run `py -3 tools/run.py installed-skills --apply` and `py -3 tools/run.py mesh --apply`.
- [ ] Run the focused workflow-contract tests.
- [ ] Run one GREEN pressure trial for `semantic-authority` and inspect its output against the rubric.
- [ ] Review the diff against the scope and authority invariants.
- [ ] Mark this plan `completed-awaiting-retirement` with every agent-owned item checked.
- [ ] Stage the intended tree and make a normal hooked commit.
- [ ] Push `codex/semantic-authority` and open a Draft PR into `main`.
- [ ] Verify the remote PR head matches the committed local head and report the PR URL.
