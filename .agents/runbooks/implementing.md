...
- **Runtime subagent profiles synced:** If `.agents/agents/reviewer-*.md` or other subagent profiles were added or changed, run `tools/run runtime-agents --apply --allow-shared-checkout` to stage them in the main checkout. Restart the IDE before dispatching `run_subagent` with the new profiles.
