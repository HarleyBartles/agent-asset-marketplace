...
PROJECTION_ROOT = ROOT / "codex-marketplace/plugins/superpowers"
...
def _entry_overlay_root(entry: dict[str, Any]) -> Path | None:
    ...
    if not isinstance(overlay_path, str) or not overlay_path.strip():
        raise ValueError(f"superpowers entry {entry.get('canonical_name')} has an invalid adaptation_overlay_path")
...
def _validate_entry(entry: dict[str, Any]) -> None:
    ...
    if not isinstance(canonical_name, str) or not canonical_name:
        raise ValueError("superpowers bundle manifest entry is missing canonical_name")
    ...
    if source_category == "third_party" and content_mode == "adapted":
        if overlay_root is None:
            raise ValueError(f"superpowers adapted entry {canonical_name} requires adaptation_overlay_path")
...
def _materialize_entry(entry: dict[str, Any], *, write: bool) -> None:
    ...
    try:
        ...
        if expected_files != actual_files:
            raise ValueError(
                f"superpowers projection mismatch for {entry['canonical_name']}: file inventory differs from reconstruction"
            )
        for rel_path in expected_files:
            ...
            if expected_bytes != actual_bytes:
                raise ValueError(
                    f"superpowers projection mismatch for {entry['canonical_name']}: file content differs at {rel_path}"
                )
    finally:
        tempdir.cleanup()
...
def reconcile_superpowers_projection(*, write: bool) -> None:
    ...
    for entry in entries:
        if not isinstance(entry, dict):
            raise ValueError("superpowers bundle manifest entries must contain objects")
...
def main() -> int:
    ...
    if args.check:
        print("OK superpowers projection: codex-marketplace/plugins/superpowers/skills")
    else:
        print("OK superpowers projection materialized: codex-marketplace/plugins/superpowers/skills")
    return 0
