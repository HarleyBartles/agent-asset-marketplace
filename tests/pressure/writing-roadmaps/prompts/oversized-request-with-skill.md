...
Follow the `writing-roadmaps` skill: detect the epic scope, run `handoff-gates` spec-readiness, create a roadmap at `.agents/superpowers/roadmaps/YYYY-MM-DD-ecommerce-site.md`, use `writing-plans` to write Plan 1, then run `handoff-gates` plan-readiness and report the final rating.
