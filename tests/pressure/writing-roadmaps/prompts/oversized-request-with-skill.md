...
human-supplied request as the design authority, create a roadmap at
`.agents/plans/ecommerce-site/roadmap.md`, use `writing-plans` to write Plan 1,
then run `handoff-gates` plan-readiness and report the current rating without
storing it in the roadmap.

# Expected pass
The agent creates a roadmap and a first plan rather than one giant plan, and
hands off only after plan-readiness meets the floor.
