...
for f in conflicted:
    ...
    elif 'house-skills' in f and f.endswith(('.json', '.md')):
        # These are the registry/provenance files we fixed - take our version
        to_checkout_theirs.append(f)
    elif f.startswith('provenance/'):
        # Our provenance fix - take our version
        to_checkout_theirs.append(f)
    else:
        to_checkout_ours.append(f)
