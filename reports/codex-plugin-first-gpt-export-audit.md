...
| `superpowers-plus` | Third-party retained upstream `obra/superpowers` `v5.1.0` plus the first-party `linear-superpowers` projection. | Exposed through `codex-marketplace/plugins/superpowers-plus/`. | Mix of `direct`, `overlay`, and `excluded` exports. | `7` overlay-backed skills, `5` direct skills, `3` excluded skills. Source-only harness surfaces stay in third-party custody. | `SOURCE.md`, `PROJECTION.md`, `references/bundle-manifest.json`, and `references/provenance-map.json` describe the source/projection split truthfully. | None. |
