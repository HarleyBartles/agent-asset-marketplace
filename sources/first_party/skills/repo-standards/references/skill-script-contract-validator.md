...
1. Run the failing script with `--help` and `--check` locally:
   ```bash
   py -3 sources/first_party/skills/<skill-name>/scripts/<script>.py --help
   py -3 sources/first_party/skills/<skill-name>/scripts/<script>.py --check
   ```
...
4. Re-run the validator:
   ```bash
   py -3 sources/first_party/skills/repo-standards/scripts/validate_skill_scripts.py
   ```
