...
Do not use this reference to dispatch workers, poll Linear, verify GitHub, run validation, mutate source evidence, close issues, or update repositories. Use `linear-issue-shaping` for Linear/Codex state, `using-github-mcp` for GitHub proof, and `verification-before-completion` for completion readiness.
