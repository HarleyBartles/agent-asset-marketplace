...
| `approved_plan_execution_ready` | Approved plan is merged to `main`, plan path/PR/commit evidence exists, staleness check passes | The approved plan is ready to execute | Hand to `repo-worker-base` -> matching baseline + local `.agents/guides/` guide -> `/using-superpowers-plus` with execution context | Phase 3 |
...
- `approved_plan_execution_ready` -> `repo-worker-base` + matching baseline/local guide -> `/using-superpowers-plus` with execution context. `/using-superpowers-plus` owns Superpowers lane choice (SDD, TDD, or direct implementation)
...
- `repo_worker_coding` -> `repo-worker-base` + matching baseline/local guide -> `/using-superpowers-plus` with the discovered mode (existing)
