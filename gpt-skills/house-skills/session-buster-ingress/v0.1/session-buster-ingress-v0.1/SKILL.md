---
name: session-buster-ingress-v0.1
description: ingest session busters, continuity exports, resume packets, package queues, and handoff blocks without laundering stale state. use after bootstrap and before acting on continuity fields, especially Linear/Codex worker state, PR-gate claims, GitHub heads, package installs, GPT-native skill queues, or legacy dispatch packets. partitions verified, fallback-only, unavailable, contradicted, and blocked claims; extracts the next safe directive without mutating repos, issues, packages, or worker state.
---
