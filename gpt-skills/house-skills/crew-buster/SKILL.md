---
name: crew-buster
description: 'Apply Crew thinking roles to a concrete plan, route choice, issue shape, skill update, failure-mode question, or durable-fix prompt. Use for introspective planning prompts such as what is your plan, why that route, are you sure, how do we make it boring, what is the durable fix, what else would you add, or do you agree. Owns planning and route interrogation only: preserve the Linear/Codex default coding workflow, route GPT-native skillwork to the skill stack, and defer execution/proof to specialist skills.'
version: v1
...
Use `crew-v1-buster` to apply the Crew to a specific plan, implementation topic, issue shape, route choice, failure-mode question, or durable-fix prompt before action.
...
`crew-v1` is the doctrine source for the six Crew thinking roles, proper-noun role names, TPS handoff discipline, and the golden-plan contract. The Crew are GPT thinking roles, not autonomous agents or workers.

Read `skills://crew-v1/SKILL.md` when the role model, dependency chain, TPS handoff, or golden-plan proof materially matters. Do not copy Crew doctrine into this skill. Use this skill to apply that doctrine to the current task.
...
- `linear_codex_candidate`: repo-backed coding work that Codex Cloud can execute from an accessible repo environment. Next owner: `worker-dispatch-linear-v1`.
- `gpt_native_skillwork`: installed ChatGPT skill or doctrine work not proven repo-backed. Next owner: `skill-creator` -> `skill-validator-v1` -> `skill-packager-v1` -> `skill-buster-v0.1`.
- `repo_backed_skill_source`: skill/plugin/source work in a known editable repo. Next owner: `worker-dispatch-linear-v1` only after repo path, publication path, and validation route are known.
...
- Use formal mode when the user explicitly asks to run crew-v1-buster, asks for a structured Crew assessment, or when the plan has significant mutation, Linear/Codex, issue, skill, repo, artifact, or closeout consequences.
...
2. If the full dependency model matters, load `crew-v1` and apply the roles in order: Index, Silk, Writ, Klause, Rollback, Receipt.
...
- Use `worker-dispatch-linear-v1` for normal coding dispatch, Codex worker status, Linear issue handoff, PR-gate handling, and Linear/Codex state checks.
...
- Use `tps-reporting-v1` for report partitioning.
...
- Use `skill-creator`, `skill-validator-v1`, `skill-packager-v1`, and `skill-buster-v0.1` for GPT-native skill creation, validation, packaging, and handoff.
