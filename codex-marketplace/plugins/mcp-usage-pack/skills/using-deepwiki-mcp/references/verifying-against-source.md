...
1. For code-level facts, open the actual file with `using-github-mcp`, `webfetch`, or a local file read.
1. For version or dependency facts, read `package.json`, `pyproject.toml`, or the equivalent in the actual repo.
1. For security or deployment decisions, confirm with live source or ask your human partner.
