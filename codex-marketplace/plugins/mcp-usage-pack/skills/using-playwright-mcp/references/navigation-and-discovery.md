...
1. **Use `browser_find` before `browser_snapshot`.** It is cheaper when you only need a ref.
1. **Ref values come from `browser_snapshot` or `browser_find`.** Pass the exact `ref` as `target` for click/type actions.
