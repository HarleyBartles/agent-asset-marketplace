...
3. Send `task` as a compact task description and `candidates` as objects with exact live `id` and accurate `description`. Add `constraints`, `stakes`, and `priorities` only when grounded in the task or known runtime facts. Do not manufacture price, latency, entitlement, or context-size claims.
4. Accept only a returned candidate ID from the supplied set. Recheck live availability, policy, reasoning, and context restrictions immediately before dispatch. If the result is uncertain, malformed, unavailable, or conflicts with policy, use the existing deterministic/manual route and record why.
