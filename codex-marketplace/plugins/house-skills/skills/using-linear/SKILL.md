...
| Work with teams, labels, statuses, cycles, milestones, customers, or status updates | [`references/admin-metadata.md`](references/admin-metadata.md) |
| Archive or delete a status update | [`references/destructive.md`](references/destructive.md) |
| Need the complete callable surface | [`references/surface-map.md`](references/surface-map.md) |

## Fast rule

If you are looking for a create or update tool and do not see `create_*` or `update_*`, search `save_*` first. The surfaced create-only exception here is `_create_issue_label`.
