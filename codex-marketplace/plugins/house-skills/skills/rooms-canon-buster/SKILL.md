...
Use `rooms-project-doctrine-v1` for Rooms truth boundaries, `rooms-source-partitioning-v1` to separate evidence classes, `rooms-ambiguity-buster-v1` when the right outcome is to preserve unresolved identity or motive, and `worker-dispatch-linear-v2` when a canon-safe next step turns into repo-backed work, issue handling, or publication.
...
- `worker-dispatch-linear-v2` for repo-backed work, publication, and issue closure posture.
