...
### Documentation
- provenance and source mapping in `SOURCE.md` (see `## Retention note` and `## Plugin custody`)
...
- Editable custody lives in `codex-marketplace/plugins/superpowers-plus/skills/<name>/`. The upstream
  `obra/superpowers` v6.2.0 MIT snapshot is recorded in `SOURCE.md` (see `## Retention note` and `## Plugin custody`) as reference only; it is not the editable
