...
metadata:
  ...
  use_when:
    - a Codex or Devin Desktop Superpowers session needs evidence-backed analysis.
    - the human requests a scrubbed diagnostic bundle or issue draft after analysis.
  do_not_use_when:
    - the goal is to diagnose or change Superpowers rather than report session evidence.
    - the session is from an unsupported harness.
  related_skills:
    - context-safety
...
1. **Locate.** Resolve each session to verified absolute filesystem paths using `references/session-discovery.md`. Confirm a past session by quoting its first prompt and timestamp, and list every candidate you rejected with the reason, or "none". Enumerate subagent transcripts. Create a repository-segregated off-repo diagnostic workspace through the host or repository scratch policy, tell your partner the path, and fill `templates/case.md` there, following its provenance rules for environment and skill observations.
1. **Triage.** Read the region around the reported problem yourself. Then use `selecting-a-subagent` and `dispatching-parallel-agents` when delegation is authorized, dispatching one analyst subagent per dimension in parallel, each given the case file path, `prompts/analyst-common.md`, and one dimension file from `prompts/`: `skill-timeline.md`, `plan-adherence.md`, `repeated-work.md`, `stumbles.md`, `quality-evidence.md`, `request-conflicts.md`, `cost-and-time.md`. Split a dimension by turn range when the transcript is long. Dispatch routing does not itself authorize delegation. When runtime or task policy forbids fan-out, use a clearly disclosed sequential/self-analysis fallback that applies the same seven prompts. Do not claim the analysts ran when they did not. Discard any returned finding without `path:line`.
1. **Report.** Fill every section of `templates/report.md` in order, write it to the workspace, show it, and give the path. Check what cited content actually proves and preserve the supporting case; a symlink alias is not a redundant copy.
1. **GitHub issues** — when report §7 says possible or likely, or your partner asks. Route GitHub inspection through `using-github-mcp` and any mutation through `connector-safety`. Search open and closed issues for the symptoms per `references/github-issues.md`. Show matches and suggest adding the report to the closest. If none match, fill `templates/issue.md`, write it to the workspace, show the exact text, and create the issue only after approval. `gh` cannot attach files; if a bundle exists, give your partner its path to attach in the browser.
1. **Export** — route sensitive filesystem or archive side effects through `connector-safety`. Only proceed when your partner asks for a bundle; never build one unprompted. If the intake goal was a bug report, say once that a scrubbed bundle is available on request, then wait. Ask the redaction level, stating what each includes: skeleton (no tool-result bodies), evidence (bodies only for cited events), full. Build the bundle per `templates/bundle-README.md`, dispatch `prompts/scrub.md`, then `prompts/scrub-audit.md`, repeating both until the audit returns CLEAN. Complete the bundle template's evidence check and reconciliation before showing the final scrub log, file list, and privacy and evidence outcomes. Archive (`zip -r` or `tar -czf`) only after approval. With the archive path, state what it contains, point at the scrub log for replacements, and say scrubbing can miss things: they must review every file before sharing.
1. **Similar sessions** — when asked. Turn confirmed findings into a signature, list candidates by mtime and size, find marker line numbers, and apply `prompts/similar-session.md` per candidate. When delegation is authorized, dispatch candidates in parallel through the existing routing owners; otherwise use the same disclosed sequential/self-analysis fallback and do not claim candidate analysts ran. Append report §9.

## Quick reference

All seven analytical dimensions always run, whether through authorized analyst subagents or the disclosed sequential/self-analysis fallback. This table says which region to read yourself in step 3 and which findings to lead with in the verdict.
...
- **Context safety.** One transcript line can be a megabyte. Invoke `context-safety` and follow `references/context-safety.md` on every session file, every time.
