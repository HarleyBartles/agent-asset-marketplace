...
- The active plugin includes the first-party Superpowers+ skills, including the `using-superpowers-plus` workflow-selection entrypoint, plus the compositional helper skills (`handoff-gates`, `inspecting-the-environment`, `requesting-code-review`, `selecting-a-subagent`, `working-with-epics`).
- The retained `obra/superpowers` v6.2.0 MIT snapshot is recorded in `provenance/superpowers-plus.md` for reference; it is not the editable surface and no adapter overlay is applied.
