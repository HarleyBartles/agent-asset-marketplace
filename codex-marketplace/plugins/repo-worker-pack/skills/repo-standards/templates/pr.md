...
- For a normal commit, let the tracked pre-commit hook run `tools/run.py precommit --apply` as the single complete local gate. Do not run the full `ci --check` before a normal commit.
