...
Canonical: `py -3 tools/run.py ci --check` and `py -3 tools/run.py marketplace --apply`. Use `tools/run runtime-agents --apply` to stage new subagent profiles in the main checkout when working in a worktree; see `docs/non-repo-locations-policy.md`.
